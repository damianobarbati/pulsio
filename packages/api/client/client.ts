...
const script = (document.currentScript as HTMLScriptElement | null) || document.querySelector<HTMLScriptElement>('script[data-pulsio-id]');
const endpoint = new URL(script ? script.src : 'http://localhost:8080/client.js').origin;
const userId = script ? script.dataset.pulsioId : '';
...
let pageId = Date.now().toString(36) + Math.random().toString(36).substring(2);
...
const send = (name: string, options: TrackOptions = {}, engagement?: { e: number; sd: number }) => {
  ...
  if (new TextEncoder().encode(body).length <= 2000) navigator.sendBeacon(`${endpoint}/event`, body);
};
const engagement = () => {
  updateScroll();
...
const pageview = () => {
  ...
  referrer = pageUrl;
  pageUrl = nextUrl;
  pageId = Date.now().toString(36) + Math.random().toString(36).substring(2);
  scrollDepth = 0;
