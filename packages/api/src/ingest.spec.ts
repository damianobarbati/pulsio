...
const countEvents = async (eventId?: string) => {
  const query = eventId ? 'SELECT count() AS count FROM events FINAL WHERE id = {eventId:UUID}' : 'SELECT count() AS count FROM events FINAL';
  const result = await clickhouse.query({ query, query_params: eventId ? { eventId } : {}, format: 'JSONEachRow' });
...
describe('Redis to ClickHouse', () => {
  ...
  it('retains the batch after an insert fails, then retries without double counting', async () => {
    ...
    await cache.rpush(queueName, JSON.stringify(event));
    await processQueue();
    expect(await countEvents(event.id)).toEqual(1);
