...
const exchangeRate = async ({ date, currency }: { date: string; currency: string }) => {
  if (currency === 'USD') return 1;
  const key = `exchange-rate:usd:${date}:${currency}`;
  const cached = await cache.get(key);
  if (cached) return Number(cached);
  const response = await fetch(`https://api.frankfurter.dev/v2/rates?base=usd&date=${date}`);
  if (!response.ok) throw new Error(`Exchange rate unavailable for ${date}`);
  const payload = (await response.json()) as unknown;
  const rows = Array.isArray(payload) ? payload : [];
  const row = rows.find(
    (item): item is { quote: string; rate: number } =>
      Boolean(item) && typeof item === 'object' && (item as { quote?: unknown }).quote === currency && typeof (item as { rate?: unknown }).rate === 'number',
  );
  if (!row) throw new Error(`Exchange rate unavailable for ${currency} on ${date}`);
  await cache.set(key, String(row.rate));
  return row.rate;
};
...
const buildQuery = (input: AnalyticsQuery) => {
  ...
  const cte = `WITH raw AS (
    SELECT *, lagInFrame(toNullable(timestamp), 1, NULL) OVER (PARTITION BY fingerprint ORDER BY timestamp, event_name ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS previous
    FROM events WHERE site_id = {siteId:String} AND timestamp >= toStartOfDay(parseDateTime64BestEffort({from:String})) AND timestamp < parseDateTime64BestEffort({to:String})
  ), numbered AS (
    SELECT *, sum(if(previous IS NULL OR dateDiff('second', previous, timestamp) >= 1800, 1, 0)) OVER (PARTITION BY fingerprint ORDER BY timestamp, event_name) AS session FROM raw
  ), sessions AS (
    SELECT fingerprint, session, countIf(event_name = 'pv') AS session_views,
      ...
      argMinIf(path, tuple(timestamp, event_name), event_name = 'pv') AS entry_page,
      argMaxIf(path, tuple(timestamp, event_name), event_name = 'pv') AS exit_page,
      argMin(url, tuple(timestamp, event_name)) AS entry_url,
      argMin(source, tuple(timestamp, event_name)) AS visit_source,
      argMin(channel, tuple(timestamp, event_name)) AS visit_channel,
      argMin(ifNull(referrer, ''), tuple(timestamp, event_name)) AS visit_referrer
...
export const AnalyticsRepository = {
  ...
  async revenue(input: AnalyticsQuery & { conversion?: Goal }) {
    ...
    const rows = await queryRows<{ date: string; currency: string; totalRevenue: number; orders: number }>({
      params,
      query: `${cte} SELECT toDate(timestamp) AS date, revenue_currency AS currency, toFloat64(sum(revenue_amount)) AS totalRevenue, count() AS orders FROM filtered WHERE revenue_amount IS NOT NULL AND revenue_currency != '' ${condition} GROUP BY date, revenue_currency ORDER BY date, currency`,
    });
    let totalRevenue = 0;
    let orders = 0;
    for (const row of rows) {
      const sourceRate = await exchangeRate({ date: row.date, currency: row.currency });
      const reportingRate = await exchangeRate({ date: row.date, currency: input.reportingCurrency });
      totalRevenue += (row.totalRevenue / sourceRate) * reportingRate;
      orders += row.orders;
    }
    return orders ? [{ currency: input.reportingCurrency, totalRevenue, averageRevenue: totalRevenue / orders, orders }] : [];
  },
