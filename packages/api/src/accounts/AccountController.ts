...
export const AccountController = {
  ...
  async register(input: { email: string; password: string; domain: string; plan: 'start' | 'grow' | 'scale' }) {
    try {
      ...
      return { token, setup: AccountService.setup({ site: sites[0] }) };
    } catch (error) {
  ...
  async account({ account }: { account: Account }) {
    ...
    return { email: account.email, sites: sites.map((site) => AccountService.setup({ site })), trial_ends_at: new Date(stored.trial_ends_at).toISOString() };
  },
  async setup({ account }: { account: Account }) {
    const sites = await AccountRepository.sites({ accountId: account.id });
    if (!sites.length) throw new AppError(404, 'SITE_NOT_FOUND', 'Add a website first.');
    const result = AccountService.setup({ site: sites[0] });
    return result;
  },
  async addSite({ account, domain }: { account: Account; domain: string }) {
    try {
      ...
      const result = AccountService.setup({ site });
      return result;
    } catch (error) {
