...
export const AccountService = {
  normalizeDomain({ domain }: { domain: string }) {
    ...
    if (!['http:', 'https:'].includes(url.protocol) || !url.hostname || url.hostname.length > 253 || !/^[a-z0-9.-]+$/i.test(url.hostname))
  ...
  async register({ id, email, password, domain, plan = 'start' }: { id?: string; email: string; password: string; domain: string; plan?: 'start' | 'grow' | 'scale' | 'expand' }) {
  ...
  setup({ site, userId }: { site: SiteRow; userId: string }): SiteSetup {
    return {
      id: site.id,
      domain: site.domain,
      detected: Boolean(site.detected_at),
      automaticallyDiscovered: site.automatically_discovered,
      snippet: `<script async src="${ENV.API_URL}/client.js" data-pulsio-id="${userId}"></script>`,
