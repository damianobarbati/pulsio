...
export const AccountRepository = {
  async create({
    ...
    const account = await database.transaction(async (transaction) => {
      const [user] = await transaction('users')
        ...
        .returning(['id', 'email']);
      await transaction('sites').insert({ user_id: user.id, domain });
      await transaction('subscriptions').insert({ account_id: user.id, plan, interval: 'month', status: 'trialing' });
      return user;
    });
    return account;
  },
  async findByEmail({ email }: { email: string }) {
    const account = await database<AccountRow>('users').where({ email }).first();
    return account;
  },
  async findById({ accountId }: { accountId: string }) {
    const account = await database<AccountRow>('users').where({ id: accountId }).first();
    return account;
  },
  ...
  async updateLastLogin({ accountId }: { accountId: string }) {
    await database('users').where({ id: accountId }).update({ last_login_at: new Date() });
  },
  ...
  async delete({ accountId }: { accountId: string }) {
    await database.transaction(async (transaction) => {
      ...
      await transaction('users').where({ id: accountId }).delete();
    });
  },
  async sites({ accountId }: { accountId: string }) {
    const sites = await database<SiteRow>('sites').where({ user_id: accountId }).orderBy('created_at', 'asc');
    return sites;
  },
  async addSite({ accountId, domain }: { accountId: string; domain: string }) {
    const [site] = await database<SiteRow>('sites').insert({ user_id: accountId, domain }).returning('*');
    return site;
  },
  async discoverSite({ userId, domain }: { userId: string; domain: string }) {
    const [site] = await database<SiteRow>('sites').insert({ user_id: userId, domain, automatically_discovered: true }).returning('*');
    return site;
  },
