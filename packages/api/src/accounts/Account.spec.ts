...
describe('Account onboarding', () => {
  ...
  it('registers, detects a real beacon, supports a second website, and logs in again', async () => {
    ...
    expect(setup).toMatchObject({ domain: 'example.com', detected: false, snippet: `<script defer src="${ENV.API_URL}/client.js" data-site="example.com"></script>` });
    expect(registration.headers.get('set-cookie')).toContain('HttpOnly');
    const event = await fetch(new URL('/event', baseUrl), {
      method: 'POST',
      headers: { 'content-type': 'text/plain;charset=UTF-8' },
      body: JSON.stringify({ v: '1', n: 'pv', s: 'example.com', u: 'https://example.com/about', r: '', w: 1920, l: 'en', t: 'UTC' }),
    });
  ...
  it('resets only the selected site data and deletes the account', async () => {
    ...
    const firstSite = (await first.clone().json()).id;
    const secondSite = (await second.clone().json()).id;
    for (const domain of ['first.example.com', 'second.example.com']) {
      await fetch(new URL('/event', baseUrl), {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ v: '1', n: 'pv', s: domain, u: `https://${domain}/`, r: '', w: 1920, l: 'en', t: 'UTC' }),
      });
    }
