...
if (!websiteUrl || !dashboardUrl || !superadminUrl || !superadminEmail || !superadminPassword)
  throw new Error('WEBSITE_URL, WEBAPP_URL, SUPERADMIN_URL, SUPERADMIN_EMAIL, and SUPERADMIN_PASSWORD are required.');
const runId = randomUUID();
const email = `jane.dane+${runId}@example.test`;
const password = 'jane.dane@gmail.com';
const domain = `happy-${runId}.example.test`;
...
const removeAccount = async () => {
  await pg.transaction(async (transaction) => {
    const account = await transaction('users').where({ email }).first<{ id: string }>();
    if (!account) return;
    await transaction('payments').where({ account_id: account.id }).delete();
    await transaction('users').where({ id: account.id }).delete();
  });
};
...
describe('Happy path', () => {
  it('registers Jane Dane, opens the dashboard, and pays for Start', async () => {
    ...
    await Promise.all([current.waitForURL(`${dashboardUrl}/**`), current.getByRole('link', { name: 'Open dashboard ↗' }).click()]);
    ...
    await superadmin.getByRole('textbox', { name: 'Username' }).fill(superadminEmail);
