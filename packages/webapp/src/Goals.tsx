...
import type { GoalStats } from 'types/analytics.ts';
import { formatMetric } from './format.ts';
import { IEdit, IPlus, ITrash } from './icons.tsx';
import { goalMutation } from './requests.ts';

type GoalsProps = { siteId: string; goals: GoalStats[]; onChanged: () => void; onSelect: (id: string) => void };
type GoalForm = { name: string; kind: 'page' | 'event' | 'scroll'; target: string; threshold: number; properties: string };
export const Goals = ({ siteId, goals, onChanged, onSelect }: GoalsProps) => {
  const [editing, setEditing] = React.useState<string | null>(null);
  const [deleting, setDeleting] = React.useState<string | null>(null);
  const [message, setMessage] = React.useState('');
  const form = useForm<GoalForm>({ defaultValues: { name: '', kind: 'event', target: '', threshold: 50, properties: '{}' } });
  ...
  const submit = form.handleSubmit(async ({ properties, ...values }) => {
    try {
    ...
    <section className="rounded-lg border border-gray-100 bg-white p-6 shadow-sm md:col-span-2">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          onClick={() => {
            form.reset({ name: '', kind: 'event', target: '', threshold: 50, properties: '{}' });
            setEditing('');
            setMessage('');
          }}
        >
      ...
      {editing !== null && (
        <form onSubmit={submit} className="mt-5 grid gap-4 rounded-lg border border-violet-100 bg-violet-50/50 p-5 sm:grid-cols-2">
          ...
          <label className="text-sm">
            ...
            <input {...form.register('properties')} placeholder={'{"plan":"pro"}'} className="mt-1 w-full rounded border border-gray-200 bg-white p-2 font-mono" />
          </label>
                ...
                <td className="px-2">
                  <div className="flex gap-3">
                    <button
                      ...
                      onClick={() => {
                        form.reset({ ...goal, properties: JSON.stringify(goal.properties) });
                        setEditing(goal.id);
                        setMessage('');
                      }}
                    >
