...
import { IChrome, IExpand, IFirefox, IGithub, IGlobe, IGoogle, ILink, ISafari } from 'ui/icons.tsx';
import { toDuration, toNumber, toRate } from '../helpers.ts';

type AnalyticsFilter = any;
type BreakdownRow = any;
type Dimension = any;
...
export const Report = ({ className, query, tabs, initial, color, onFilter }: ReportProps) => {
  const form = useForm({ defaultValues: { campaign: 'utm_campaign' } });
  ...
  const report = useSWR<BreakdownRow[]>(`/analytics/breakdown?${query}&dimension=${apiDimension}${property ? `&key=${encodeURIComponent(property)}` : ''}`, GET);
  ...
  return (
    <FormProvider {...form}>
      <section className={cx('min-w-0 rounded-lg border border-gray-100 bg-white p-5 shadow-sm sm:p-6', expanded ? 'md:col-span-2' : '', className)}>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-2" role="tablist" aria-label={`${tabs[0].label} reports`}>
            {tabs.map((tab) => (
              <button
                ...
                className={cx(
                  'border-b-2 pb-4 font-semibold text-xs uppercase tracking-wide',
                  dimension === tab.dimension ? 'border-gray-800 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-800',
                )}
              >
                {tab.label}
              </button>
        ...
        {active === 'map' && map.data ? (
          <>
            ...
            <a className="text-gray-400 text-xs" href="https://db-ip.com" target="_blank" rel="noreferrer">
              IP Geolocation powered by DB-IP
            </a>
          </>
        ) : (
                    ...
                    <tr key={row.name}>
                      ...
                      {expanded &&
                        [
                          ...
                          toNumber(row.pageviews),
                          toNumber(row.visits),
                          toRate(row.bounceRate),
                          toDuration(row.visitDuration),
                          toDuration(row.timeOnPage),
