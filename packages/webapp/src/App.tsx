...
type Site = { id: string; domain: string; detected: boolean; snippet: string; adminUrl: string };
...
export const App = () => {
  ...
  const [siteId, setSiteId] = React.useState(new URLSearchParams(location.search).get('site') || '');
  ...
  if (!account.data && account.error && account.error.status !== 401)
    return (
      <main className="mx-auto max-w-lg p-10">
        ...
        <button type="button" onClick={() => account.mutate()} className="mt-5 rounded-full bg-emerald-950 px-6 py-3 text-white">
          Try again
        </button>
      </main>
    );
      ...
      <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-12">
        ...
        <form onSubmit={submitLogin} className="mt-8 space-y-5">
          <label className="block font-semibold text-sm">
            Email
            <input type="email" autoComplete="email" required {...loginForm.register('email')} className="mt-2 w-full rounded-xl border border-emerald-950/25 bg-white px-4 py-3" />
          </label>
          <label className="block font-semibold text-sm">
            Password
            <input
              ...
              className="mt-2 w-full rounded-xl border border-emerald-950/25 bg-white px-4 py-3"
            />
          </label>
          {login.error && (
          ...
          <button disabled={login.isMutating} className="w-full rounded-full bg-emerald-950 px-6 py-3 font-semibold text-white disabled:opacity-50">
  ...
  return (
    <div className="min-h-screen">
      <header className="border-emerald-950/15 border-b bg-white">
        <div className="mx-auto flex max-w-[1440px] flex-wrap items-center justify-between gap-4 px-6 py-3">
          ...
          <nav aria-label="App navigation" className="flex gap-5 font-semibold text-sm">
            <button type="button" onClick={() => setShowSettings(false)} aria-current={!showSettings ? 'page' : undefined}>
              Analytics
            </button>
            <button type="button" onClick={() => setShowSettings(true)} aria-current={showSettings ? 'page' : undefined}>
              Websites & account
            </button>
            <button type="button" onClick={signOut} disabled={logout.isMutating}>
              Log out
            </button>
          </nav>
        </div>
      </header>
      <main className="mx-auto max-w-[1440px] px-4 py-6 sm:px-8 lg:px-12">
        {logout.error && (
        ...
        {showSettings ? (
          <section className="max-w-2xl">
            ...
            <section className="mt-8 rounded-2xl border border-emerald-950/15 bg-white p-6">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="font-semibold text-sm">
                  ...
                  <select
                    value={plan}
                    onChange={(event) => setPlan(event.target.value as Billing['plan'])}
                    className="mt-2 block w-full rounded-xl border border-emerald-950/25 px-4 py-3 font-normal"
                  >
                ...
                <label className="font-semibold text-sm">
                  ...
                  <select
                    value={interval}
                    onChange={(event) => setInterval(event.target.value as Billing['interval'])}
                    className="mt-2 block w-full rounded-xl border border-emerald-950/25 px-4 py-3 font-normal"
                  >
