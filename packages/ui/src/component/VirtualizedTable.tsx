...
export const VirtualizedTable = <T extends object>({
  ...
  return (
    <div ref={parentRef} className={cx('h-full min-w-0 overflow-auto', className)} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)}>
      <table className="table-fixed border-separate border-spacing-0 text-left text-sm" style={{ width: `max(100%, ${table.getTotalSize()}px)` }}>
        ...
        <tbody>
          ...
          {visibleRows.map((row) => (
            <tr key={row.id} className={cx('group h-10 transition-colors', onRowClick ? 'cursor-pointer hover:bg-pulsio-surface' : '')} onClick={() => onRowClick?.(row.original)}>
