...
type KpiCardProps = { label: string; value: string | number; detail?: string; delta?: number | null; live?: boolean; className?: string };
const formatDelta = (delta: number) => `${delta > 0 ? '+' : ''}${Number(delta.toFixed(1))}%`;
export const KpiCard = ({ label, value, detail, delta, live, className = '' }: KpiCardProps) => (
  <Card className={className}>
    ...
    <div className="mt-4 flex items-baseline gap-2">
      ...
      {delta !== null && delta !== undefined && (
        <span className={`font-semibold text-sm tabular-nums ${delta > 0 ? 'text-emerald-600' : delta < 0 ? 'text-red-600' : 'text-pulsio-muted'}`}>{formatDelta(delta)}</span>
      )}
    </div>
